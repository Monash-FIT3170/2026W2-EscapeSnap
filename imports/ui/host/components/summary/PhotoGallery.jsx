      ...
      >
        ...
        <div style={{ flex: '0 0 320px', padding: 24, overflowY: 'auto', maxHeight: '80vh' }}>
          <div className="flex items-start justify-between" style={{ marginBottom: 20 }}>
            <div>
              <p style={{ fontSize: 10, letterSpacing: '1px', color: MUTED }}>ROUND {photo.roundNumber}</p>
          ...
          <div style={{ borderLeft: `4px solid ${status}`, background: INNER, padding: '12px 16px', marginBottom: 20 }}>
            ...
            {photo.attemptNumber > 1 && (
              <p style={{ fontSize: 10, letterSpacing: '0.5px', color: DIM, marginTop: 4 }}>
                ATTEMPT {photo.attemptNumber}
              </p>
            )}
          </div>

          <p style={{ fontSize: 10, letterSpacing: '1px', color: MUTED }}>TARGET OBJECT</p>
          ...
          <p style={{ fontSize: 10, letterSpacing: '1px', color: MUTED, marginBottom: 8 }}>DETECTIONS</p>
          {detections.length === 0 ? (
            <p style={{ fontSize: 11, color: DIM, marginBottom: 20 }}>NOTHING DETECTED</p>
          ) : (
          ...
          <p style={{ fontSize: 10, letterSpacing: '1px', color: MUTED }}>CAPTURED</p>
    ...
    <div style={{ background: PANEL, padding: 24 }}>
      ...
      {photos.length === 0 ? (
        photosExpired ? (
          ...
          <EmptyPanel
            title="PHOTO ARCHIVE PURGED"
            detail={'IMAGES ARE RETAINED FOR 6 HOURS AFTER A MISSION, THEN AUTOMATICALLY DELETED.\nALL MISSION STATISTICS ABOVE REMAIN ON RECORD.'}
          />
        ) : (
          <EmptyPanel
            title="NO PHOTOS ON RECORD"
            detail="NO CAPTURES WERE SUBMITTED DURING THIS MISSION."
          />
        )
      ) : (
        <>
          <div className="flex flex-wrap items-center gap-2" style={{ marginBottom: 16 }}>
            <span style={{ fontSize: 10, letterSpacing: '1px', color: DIM, marginRight: 4 }}>OUTCOME</span>
            ...
            <span style={{ fontSize: 10, letterSpacing: '1px', color: DIM, margin: '0 4px 0 16px' }}>OPERATIVE</span>
          ...
          {visible.length === 0 ? (
            <EmptyPanel title="NO PHOTOS MATCH THE CURRENT FILTER" />
          ) : (
            <div style={{
              display: 'grid',
